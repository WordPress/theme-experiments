		...
		public function admin_notice() {
			...
			<div class="notice notice-warning require-gutenberg-notice-wrapper notice-alt<?php echo ( $active_step ) ? ' active-step-' . esc_attr( $active_step ) : ''; ?>">
				...
				<div class="require-gutenberg require-gutenberg-install">
					...
					<p><button class="button" onclick="wpThemesRequireGutenberg.installPlugin();" aria-label="<?php esc_attr_e( 'Install Gutenberg', 'twentytwentyone-blocks' ); ?>"><?php esc_html_e( 'Install Gutenberg', 'twentytwentyone-blocks' ); ?></button></p>
				</div>
				<div class="require-gutenberg require-gutenberg-activate">
					<p><?php esc_html_e( 'The Gutenberg plugin is installed but not activated, or the "Full Site Editing" experiment is not enabled. Click the button below to enable the plugin and experiment.', 'twentytwentyone-blocks' ); ?></p>
					<p><button class="button" onclick="wpThemesRequireGutenberg.activatePlugin();"><?php esc_html_e( 'Activate Plugin & Experiment.', 'twentytwentyone-blocks' ); ?></button></p>
				</div>
				<div class="require-gutenberg require-gutenberg-success">
					<p><?php esc_html_e( 'Congratulations! All steps required were completed. Enjoy your Full Site Editing experience.', 'twentytwentyone-blocks' ); ?></p>
				</div>
			</div>
			<?php
		}
			...
			wpThemesRequireGutenberg = {
				...
				installPlugin: function() {
					...
					jQuery( '.notice .require-gutenberg-install .button' )
						.html( '<?php esc_html_e( 'Installing Gutenberg...', 'twentytwentyone-blocks' ); ?>' )
						.addClass( 'updating-message' )
						.attr( 'aria-label', '<?php esc_attr_e( 'Installing Gutenberg...', 'twentytwentyone-blocks' ); ?>' );
					...
					wp.updates.installPlugin( {
						...
						error: function( e ) {
							...
							jQuery( '.require-gutenberg-install' )
								.append( '<div class="error"><p><?php esc_html_e( 'An error occured:', 'twentytwentyone-blocks' ); ?></p><p>' + e.message + '</p><p><?php esc_html_e( 'Please check your browser console for more details', 'twentytwentyone-blocks' ); ?></p></div>' );
					...
					jQuery.get( ajaxurl, {
						...
						if ( 'success' === response ) {
							...
							jQuery( '.require-gutenberg-activate' )
								.append( '<div class="error"><p><?php esc_html_e( 'An error occured', 'twentytwentyone-blocks' ); ?>:</p><p><?php esc_html_e( 'Could not activate the plugin. Please go to the plugins page on your dashboard and manually activate the plugin.', 'twentytwentyone-blocks' ); ?></p></div>' );
						}
					} );
				},
			}
